...
export const TodoItem = defineComponent({
  ...
  setup(props) {
    ...
    const [handleChangeChecked, loadingRef] = useAsyncEvent((e: boolean | 'indeterminate') =>
      item$.pipe(
        take(1),
    ...
    return () => (
      <div class="h-10 flex items-center space-x-2 pr-4 group">
        {loadingRef.value ? (
          ...
          <Checkbox modelValue={itemRef.value.status === ETodoStatus.DONE} onUpdate:modelValue={handleChangeChecked} />
        )}
        <AutoTooltip
          id={itemRef.value.id}
